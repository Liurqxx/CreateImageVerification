...
public class SpecCaptcha extends Captcha {
	...
	private boolean graphicsImage(char[] strs, OutputStream out) {
		...
		try {
			...
			g.setStroke(new BasicStroke(1.3f, BasicStroke.CAP_BUTT,
			...
			AlphaComposite ac3 = AlphaComposite.getInstance(
					AlphaComposite.SRC_OVER, 0.8f);// 指定透明度
			g.setComposite(ac3);
			...
			for (int i = 0; i < strs.length; i++) {
				g.setColor(new Color(20 + num(110), 20 + num(110),
						20 + num(110)));
				// 计算坐标
				int x = i * w + sp + num(3);
				int y = h - num(3, 6);
				...
				g.setFont(font.deriveFont(num(2) == 0 ? Font.PLAIN
						: Font.ITALIC));
